...
public class MataKuliah extends KoneksiDB{
    ...
    public void select(){
        tbldosen.setColumnIdentifiers(new Object[]{"Kode Matakuliah", "Kode Kelompok Matakuliah", 
           "Tahun Kurikulum", "Kode Prodi", "Nama Matakuliah", "Semester", "SKS Teori","SKS Praktek", 
    ...
    try{
        ...
        list = new ArrayList<>();
        while(res.next()){
            kd_mk = res.getInt(kd_mk);
            kd_kmk = res.getInt(kd_kmk);
            tahun_kur = res.getString(tahun_kur);
            kd_prodi = res.getInt(kd_prodi);
    ...
    public void insert_update(){
        try{
            ...
            stat = conn.prepareStatement(query);
            stat.setInt(1, kd_mk);
            stat.setInt(2, kd_kmk);
            stat.setString(3, tahun_kur);
            stat.setInt(4, kd_prodi);
